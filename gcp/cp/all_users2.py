...
def get_service_account_last_auth(project_id: str, member: str) -> str:
    """
    Get the last authentication time for a service account.
    
    Args:
        project_id: GCP project ID
        member: Member identifier (e.g., serviceAccount:sa@project.iam.gserviceaccount.com)
        
    Returns:
        Last authentication timestamp or 'N/A'
    """
    ...
    try:
        ...
        for key in keys:
            if 'validAfterTime' in key:
                last_used_times.append(key.get('validAfterTime', ''))
        
        if last_used_times:
            return max(last_used_times)
        return 'Never'
...
def export_to_csv(results: List[Dict[str, Any]], output_file: str) -> None:
    ...
    Args:
        results: List of project IAM policies with activity data
        output_file: Output file path
    """
    import csv
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Project ID', 'Member', 'Member Type', 'Role', 'Last Activity'])
            ...
            if policy and 'bindings' in policy:
                for binding in policy['bindings']:
                    ...
                    for member in members:
                        ...
                        # Get last activity
                        last_activity = activity_data.get(member, 'N/A')
                        
                        writer.writerow([project_id, member, member_type, role, last_activity])
...
def main():
    ...
    try:
        ...
        for i, project_id in enumerate(projects, 1):
            ...
            # Collect activity data for service accounts
            activity_data = {}
            if policy and 'bindings' in policy:
                print(f"  Fetching last activity data for service accounts...")
                for binding in policy.get('bindings', []):
                    for member in binding.get('members', []):
                        if member.startswith('serviceAccount:'):
                            if member not in activity_data:
                                last_auth = get_service_account_last_auth(project_id, member)
                                activity_data[member] = last_auth
